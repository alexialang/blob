...
export class QuizGameComponent implements OnInit, OnDestroy {
  ...
  onOrderChanged(ids: number[]) {
    if (!this.currentQuestion) return;
    this.selectedAnswers = ids
      .map(id => this.currentQuestion!.answers.find(a => a.id === id)!)
      .filter(Boolean);
  }
  ...
  validateAnswer(): void {
    ...
    switch (t) {
      ...
      case 'multiple_choice': {
        ...
        ok =
          correct.length === this.selectedAnswers.length &&
      ...
      case 'right_order': {
        ...
        ok =
          correctOrder.length === chosenOrder.length &&
  ...
  private finishQuiz(): void {
    ...
    this.totalScore =
      this.totalQuestions > 0 ? Math.round((correctAnswers / this.totalQuestions) * 100) : 0;
  ...
  canValidate(): boolean {
    ...
    if (t === 'right_order')
      return this.selectedAnswers.length === this.currentQuestion.answers.length;
    if (t === 'matching') {
  ...
  private notifyRatingChanged(rating: number): void {
    ...
    window.dispatchEvent(
      new CustomEvent('quiz-rating-updated', {
        detail: ratingUpdate,
      })
    );
  }
