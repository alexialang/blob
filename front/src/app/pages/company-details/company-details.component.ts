...
export class CompanyDetailsComponent implements OnInit, OnDestroy {
  ...
  removeUserFromGroup(groupId: number, userId: number): void {
    ...
    this.alertService
      .open('Êtes-vous sûr de vouloir retirer cet utilisateur du groupe ?')
      .subscribe();
  ...
  deleteCollaborator(collaboratorId: number): void {
    ...
    this.alertService
      .open('Fonctionnalité de suppression de collaborateur à implémenter')
      .subscribe();
  }
  ...
  confirmAddMemberToGroup(): void {
    ...
    this.companyService
      .addUserToGroup(this.company.id, this.selectedGroupId, this.selectedMemberForGroup)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: result => {
          this.alertService
            .open(`Membre ajouté au groupe avec succès !`, {
              appearance: 'success',
            })
            .subscribe();
        ...
        error: error => {
          ...
          this.alertService
            .open(error.message || "Erreur lors de l'ajout du membre au groupe", {
              appearance: 'error',
            })
            .subscribe();
        },
